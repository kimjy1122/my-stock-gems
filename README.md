import streamlit as st
from pykrx import stock
import FinanceDataReader as fdr
import pandas_ta as ta
from datetime import datetime, timedelta
import pandas as pd

# --- 페이지 설정 ---
st.set_page_config(page_title="Stock Gems", page_icon="💎", layout="wide")

st.title("💎 My Stock Gems Scanner")
st.sidebar.header("설정")

# --- 분석 옵션 (스마트폰에서 조절 가능) ---
rsi_limit = st.sidebar.slider("최소 RSI 강도", 40, 70, 50)
market_type = st.sidebar.selectbox("시장 선택", ["KOSPI", "KOSDAQ"])

@st.cache_data(ttl=3600) # 1시간 동안 분석 결과 유지 (서버 부하 방지)
def run_analysis(m_type):
    today = datetime.now().strftime("%Y%m%d")
    # 1. 수급 데이터 가져오기
    df_investor = stock.get_market_net_purchase_of_equities_by_ticker(today, today, m_type)
    # 외인/기관 동반 매수 종목
    candidates = df_investor[(df_investor['외국인'] > 0) & (df_investor['기관합계'] > 0)]
    
    gems = []
    # 분석 속도를 위해 상위 20개만 정밀 분석
    for ticker in candidates.index[:20]:
        try:
            df = fdr.DataReader(ticker, (datetime.now() - timedelta(days=60)).strftime('%Y-%m-%d'))
            df['RSI'] = ta.rsi(df['Close'], length=14)
            df['MA20'] = ta.sma(df['Close'], length=20)
            
            curr = df.iloc[-1]
            if curr['RSI'] >= rsi_limit and curr['Close'] > curr['MA20']:
                name = stock.get_market_ticker_name(ticker)
                gems.append({
                    "종목명": name,
                    "코드": ticker,
                    "현재가": int(curr['Close']),
                    "RSI": round(curr['RSI'], 1),
                    "외인매수": df_investor.loc[ticker, '외국인'],
                    "기관매수": df_investor.loc[ticker, '기관합계']
                })
        except:
            continue
    return pd.DataFrame(gems)

# --- 메인 화면 실행 ---
if st.button('🚀 지금 종목 분석 시작'):
    with st.spinner('실시간 수급 및 차트 분석 중...'):
        result_df = run_analysis(market_type)
        
        if not result_df.empty:
            st.success(f"오늘의 {market_type} Gems를 찾았습니다!")
            
            # 보기 좋게 표로 출력
            st.dataframe(result_df.style.highlight_max(axis=0, subset=['RSI']), use_container_width=True)
            
            # 개별 종목 카드 형태로 출력 (모바일 최적화)
            for _, row in result_df.iterrows():
                with st.expander(f"📈 {row['종목명']} ({row['코드']}) 상세 보기"):
                    st.write(f"**현재가:** {row['현재가']:,}원")
                    st.write(f"**상승에너지(RSI):** {row['RSI']}")
                    st.write(f"**수급:** 외인 {row['외인매수']:,} / 기관 {row['기관합계']:,}")
        else:
            st.warning("조건에 맞는 종목이 없습니다. 옵션을 조절하거나 장 마감 후에 시도하세요.")
